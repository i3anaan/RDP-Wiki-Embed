...
class RDP_WIKI_EMBED_CONTENT {
    ...
    private function preRender() {
        ...
        $eFileTOC = $html->find('#rdp-we-filetoc',0);
    ...
    private function content_get(&$wpdb,$table) {
        ...
        if (empty($this->section)) {
            ...
            //Custom - only a section.
            $node = $html->getElementById($this->section);
            if(!$node){
                $msg = esc_html__('Unable to locate the section of wiki content.', 'rdp-wiki-embed');
                $this->_lastMessage = $msg;
                return;
            }

            $cursor = $html->getElementById($this->section)->parentNode();

            $this->scrub($cursor);
            $content = $cursor->outertext;

            while ($cursor->nextSibling()->tag == "p") {
                $cursor = $cursor->nextSibling();
                $this->scrub($cursor);
                $content = $content . $cursor->outertext;
            }
            ...
            $this->_contentRaw = '<body>' . $content . $source->outertext . '</body>';
        }
